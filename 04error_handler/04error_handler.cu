...
void onDevice(int *h_a, int *h_b, int *h_result) {
  ...
  HANDLER_ERROR_ERR(cudaMalloc((void**)&d_result, ARRAY_BYTES));
  ...
  HANDLER_ERROR_ERR(cudaMemcpy(h_result, d_result, ARRAY_BYTES, cudaMemcpyDeviceToHost));

  HANDLER_ERROR_ERR(cudaFree(d_a));
  HANDLER_ERROR_ERR(cudaFree(d_b));
  HANDLER_ERROR_ERR(cudaFree(d_result));
